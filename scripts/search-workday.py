...
SITEMAP_SOURCES = [
    ("https://salesforce.wd12.myworkdayjobs.com/en-US/External_Career_Site/siteMap.xml", "Salesforce"),
]
...
def main():
    ...
    for index, (url, meta) in enumerate(fallback_candidates, 1):
        ...
        if not is_california(location, external_path):
            continue
        val_min, val_max = salary
        job = {
            "role": title,
    ...
    for sitemap_url, sitemap_company in SITEMAP_SOURCES:
        ...
        if not job_urls:
            ...
            continue
        ca_urls = [u for u in job_urls if any(t in u.lower() for t in
                   ["california", "san-francisco", "los-angeles", "san-jose",
                    "palo-alto", "-ca-", "silicon-valley", "san-diego",
                    "cupertino", "santa-clara", "menlo-park"])]
        if not ca_urls:
            ca_urls = job_urls
        log(f"    → {len(job_urls)} total, {len(ca_urls)} plausibly CA — checking each")
        for job_url in ca_urls[:50]:
            ...
            if not is_california(location, ext_path):
                continue
            val_min, val_max = salary
            job = {
                "role": title,
