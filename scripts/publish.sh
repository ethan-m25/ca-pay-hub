...
if git diff --cached --quiet; then
  notify_discord "ℹ️ CA Pay Hub [$TODAY]: no changes ($ACTIVE_COUNT active)"
  exit 0
fi

git commit -m "data: CA daily update $TODAY (+$NEW_TODAY new, $ACTIVE_COUNT active)"
...
wrangler pages deploy . --project-name ca-pay-hub --branch main 2>&1 | tail -3 || true

notify_discord "✅ CA Pay Hub updated [$TODAY]
