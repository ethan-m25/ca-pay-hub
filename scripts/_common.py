#!/usr/bin/env python3
"""
ca-pay-hub/scripts/_common.py
Shared utilities for all CA Pay Hub search scripts.

CA Labor Code §432.3: employers with 15+ employees must include the pay scale
...
OUTPUT_FILE = os.path.join(SHARED_DIR, f"ca-jobs-raw-{TODAY}.txt")
DATA_FILE   = os.path.expanduser("~/ca-pay-hub/data/jobs.json")
...
_JOB_PAGE_MARKERS = [
    ...
    "salary range", "compensation range", "pay scale", "salary:", "we are looking",
...
CA_TERMS = [
    "california", "san francisco", "sf", "los angeles", "la",
    "san jose", "palo alto", "mountain view", "menlo park", "santa clara",
    "sunnyvale", "cupertino", "redwood city", "san diego", "sacramento",
    "oakland", "berkeley", "fremont", "san mateo", "burlingame",
    "santa monica", "west hollywood", "culver city", "el segundo",
    "irvine", "anaheim", "santa barbara", "san luis obispo",
    ", ca", "silicon valley", "bay area", "south bay", "east bay",
    "south of market", "soma", "financial district",
]
...
EXTRACT_PROMPT = """\
Extract ONE California job posting from the text below.
...
Return ONLY valid JSON in this exact format if a valid CA job with explicit USD salary range is found:
{{"role":"Job Title","company":"Company Name","min":80000,"max":120000,"location":"San Francisco, CA","source_url":"{url}","posted":"YYYY-MM-DD"}}

Return ONLY the word null (no quotes, no JSON) if:
- No explicit USD annual salary range or pay scale with actual dollar numbers in the PAGE TEXT
- Not a California location (SF, LA, San Jose, San Diego, Sacramento, etc.)
...
- location must be in California (SF, Bay Area, LA, San Jose, San Diego, Sacramento, remote roles that mention CA pay scale)
...
def extract_job(url, snippet, page_text, log=None):
    ...
    if not (30_000 <= val_min <= 1_500_000) or val_min >= val_max:
        return None

    if not any(t in job.get("location", "").lower() for t in CA_TERMS):
        return None
