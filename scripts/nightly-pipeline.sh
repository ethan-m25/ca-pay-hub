#!/usr/bin/env bash
# ca-pay-hub/scripts/nightly-pipeline.sh
# Nightly pipeline for CA Pay Hub. Run via crontab at 04:00 ET.
...
SCRIPTS_DIR="$HOME/ca-pay-hub/scripts"
...
log "=== CA nightly pipeline started ==="
...
# === Phase 3 discovery — hub: ca (added 2026-05-27) ===
log "--- Phase 3 discovery: hub_search_jobs ---"
python3 "$HOME/shared-scripts/hub_search_jobs.py" --hub "ca" >> "$LOG_FILE" 2>&1 || true
log "hub_search_jobs done (exit $?)"
log "--- Phase 3 discovery: hub_deep_search ---"
python3 "$HOME/shared-scripts/hub_deep_search.py" --hub "ca" >> "$LOG_FILE" 2>&1 || true
log "hub_deep_search done (exit $?)"
log "--- Phase 3 discovery: hub_search_kpmg ---"
python3 "$HOME/shared-scripts/hub_search_kpmg.py" --hub "ca" >> "$LOG_FILE" 2>&1 || true
log "hub_search_kpmg done (exit $?)"
log "--- Phase 3 discovery: hub_search_sap ---"
python3 "$HOME/shared-scripts/hub_search_sap.py" --hub "ca" >> "$LOG_FILE" 2>&1 || true
log "hub_search_sap done (exit $?)"
log "--- Phase 3 discovery: hub_search_scout ---"
python3 "$HOME/shared-scripts/hub_search_scout.py" --hub "ca" >> "$LOG_FILE" 2>&1 || true
log "hub_search_scout done (exit $?)"

# === Phase 5 classify — hub: ca (added 2026-05-28) ===
log "--- Phase 5 classify_step ---"
python3 "$HOME/shared-scripts/region_classifier/classify_step.py" \
    --hub "ca" --no-llm >> "$LOG_FILE" 2>&1 || true
log "classify_step done (exit $?)"
# === Phase 5.5 publish gate — hub: ca (added 2026-05-28) ===
...
GATE_RAW="$HOME/.openclaw/shared/ca-jobs-raw-$(date +%Y-%m-%d).txt"
if [[ -f "$GATE_RAW" ]]; then
...
log "--- healthcheck ---"
python3 "$HOME/shared-scripts/hub_pipeline_healthcheck.py" --failure-only --hub "ca" >> "$LOG_FILE" 2>&1
log "healthcheck done (exit $?)"
log "--- salary-qa ---"
python3 "$HOME/shared-scripts/hub_salary_qa.py" --hub "ca" --dry-run >> "$LOG_FILE" 2>&1
log "salary-qa done (exit $?)"


log "--- archive-jobs ---"
python3 "$HOME/shared-scripts/hub_archive_jobs.py" --hub "ca" --limit 300 >> "$LOG_FILE" 2>&1
log "archive done (exit $?)"
log "--- skill-extract ---"
python3 "$HOME/shared-scripts/run_locked.py" /tmp/ollama-model.lock \
    python3 "$HOME/shared-scripts/hub_skill_extract.py" --hub "ca" --limit 300 >> "$LOG_FILE" 2>&1
log "skill-extract done (exit $?)"
log "--- classify-new-jobs ---"
python3 "$HOME/shared-scripts/run_locked.py" /tmp/ollama-model.lock \
    python3 "$HOME/shared-scripts/hub_classify_new_jobs.py" --hub "ca" >> "$LOG_FILE" 2>&1 || true
...
log "=== CA nightly pipeline complete ==="
...
python3 "$HOME/shared-scripts/hub_pipeline_healthcheck.py" --failure-only \
  --hub ca --pipeline-exit-code "$PIPELINE_RC" || true
exit "$PIPELINE_RC"
