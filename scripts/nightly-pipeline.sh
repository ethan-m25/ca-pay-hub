...
log "--- healthcheck ---"
python3 "$HOME/shared-scripts/hub_pipeline_healthcheck.py" --hub "ca" >> "$LOG_FILE" 2>&1
...
log "--- archive-jobs ---"
python3 "$HOME/shared-scripts/hub_archive_jobs.py" --hub "ca" --limit 100 >> "$LOG_FILE" 2>&1
log "archive done (exit $?)"
log "--- skill-extract ---"
python3 "$HOME/shared-scripts/hub_skill_extract.py" --hub "ca" --limit 30 >> "$LOG_FILE" 2>&1
